let canRoll = true, //加控制，防止用户点击两次
  ...
  lottery = ['1元红包', '2元红包', '3元红包', '4元红包', '5元红包']; //放奖品
...
Page({
  ...
  onLoad(opt) {
    ...
    let product = wx.getStorageSync('__product');
    this.setData(JSON.parse(product));
    ...
    if (lotteryArrLen < 2) { //数组的奖品只有一个，扩展数组的长度到4
      ...
      for (let i = 0; i < 4; i++) {
        if (i % 2 == 1) { //这里为什么要取1是为了在默认的界面将指针放在谢谢的地方，防止别人拿着中奖的截图来要奖品
          ...
          evenArr[i] = insertThanks(); //在数组中间隔插入谢谢
        }
      }
    ...
    if (canRoll) {
      ...
      that.setData({
        ...
        setTimeout(function() {
            wx.showModal({
                title: '恭喜',
                content: '获得' + lottery[rightNum]+",下完单后请联系小淘微信(参考下面活动规则)，方便小淘发红包给您",
                showCancel: false,
